			...
			*tokenType = kInt;
			...
			if (c == '/') // parse one more number
			{
				...
				if (i == 0) {
					...
					if (sscanf(s, "%d", &intValue[2]) != 1) fprintf(stderr, "sscanf did not read!\n");
				}
				*tokenType = tripletToken;
			}
		} else
...
static struct Mesh * LoadOBJ(const char *filename) {
	...
	if (theMesh == NULL) {
		fprintf(stderr, "LoadOBJ could not allocate memory for theMesh!\n");
		return NULL;
	}
...
static Model* generateModel(Mesh* mesh) {
	...
	if (mesh->vertices) {
		...
		if (model->vertexArray == NULL) {
			fprintf(stderr, "Failed to allocate memory for model->vertexArray!\n");
			return NULL;
		}
	}
	...
	if (mesh->vertexNormals) {
		...
		if (model->normalArray == NULL) {
			fprintf(stderr, "Failed to allocate memory for model->normalArray!\n");
			return NULL;
		}
	}
	...
	if (mesh->textureCoords) {
		...
		if (model->texCoordArray == NULL) {
			fprintf(stderr, "Failed to allocate memory for model->texCoordArray!\n");
			return NULL;
		}
	}
...
Model* LoadDataToModel(
	...
	if (m == NULL) {
		fprintf(stderr, "LoadDataToModel could not allocate memory for m!\n");
		return NULL;
	}
