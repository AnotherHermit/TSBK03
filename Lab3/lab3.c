...
void updateWorld()
{
	...
	for (i = 0; i < kNumBalls; i++)
        ...
        {
			vec3 normal = ball[i].X - ball[j].X;
			float dist = Norm(normal);
...
void init()
{
    ...
    // Load shader
    shader = loadShaders("Lab3/shd/lab3.vert", "Lab3/shd/lab3.frag");
    printError("init shader");

    loadModelTexturePair(&tableAndLegs, "Lab3/obj/tableandlegsnosurf.obj", 0);
    loadModelTexturePair(&tableSurf, "Lab3/obj/tablesurf.obj", "Lab3/tex/surface.tga");
    sphere = LoadModelPlus("Lab3/obj/sphere.obj");
    ...
    for(i = 0; i < kNumBalls; i++)
    {
        sprintf(textureStr, "Lab3/tex/%d.tga", i);
	...
	for(i = 0; i < 5; i++)
		for (int j = i; j < 5; j++)
		{
			ball[nball].X = SetVector(-0.44 + 0.11*i + (2*kBallSize+0.02)*(j-i), 0, 1.6 - (2*kBallSize)*i);
			nball--;
		}	
